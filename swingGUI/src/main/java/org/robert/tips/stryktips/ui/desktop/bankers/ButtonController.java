...
/**
 * This is where the bankers are defined.
 * It's this system where the actor specifies which bakers you have from
 * for example 'Afontbladet' or 'Vi Tippar' single row.
...
class ButtonController extends JPanel implements StryktipsConstants,
                                                 ChangeListener
 {
    ...
    public ButtonController( StryktipsDocument stryktipsDocument )
    {
        ...
        BankersSystemButtonControllerHandler bankersSystemButtonControllerHandler = new BankersSystemButtonControllerHandler( stryktipsDocument );
        
        for ( int i = 0; i < NUMBER_OF_GAMES; i++ )
        {
            stryktipsButtons[ i * NUMBER_OF_GAMEOPTIONS ] = new TipsButton( GAMEVALUE_ONE, i );
            stryktipsButtons[ i * NUMBER_OF_GAMEOPTIONS ].addActionListener( bankersSystemButtonControllerHandler );
            add( stryktipsButtons[ i * NUMBER_OF_GAMEOPTIONS ] );
            
            stryktipsButtons[ i * NUMBER_OF_GAMEOPTIONS + 1 ] = new TipsButton( GAMEVALUE_TIE, i );
            stryktipsButtons[ i * NUMBER_OF_GAMEOPTIONS + 1 ].addActionListener( bankersSystemButtonControllerHandler );
            add( stryktipsButtons[ i * NUMBER_OF_GAMEOPTIONS + 1 ] );
            
            stryktipsButtons[ i * NUMBER_OF_GAMEOPTIONS + 2] = new TipsButton( GAMEVALUE_TWO, i );
            stryktipsButtons[ i * NUMBER_OF_GAMEOPTIONS + 2].addActionListener( bankersSystemButtonControllerHandler );
   ...
   /**
    ...
    {
        // Update view with bankers.
        
        for ( int i = 0; i < NUMBER_OF_GAMES; i++ )
        {
            char value = stryktipsDocument.getStryktipsSystem().getBanker().getBankersRow( i );

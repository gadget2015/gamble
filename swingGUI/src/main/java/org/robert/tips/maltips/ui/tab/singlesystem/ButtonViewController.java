...
/**
 * This is where the single system are defined.
...
class ButtonViewController extends JPanel implements ChangeListener,
                                                     MaltipsConstants
 {
    ...
    {        
        ...
        SingleSystemButtonViewControllerHandler singleSystemButtonViewControllerHandler = new SingleSystemButtonViewControllerHandler( maltipsDocument );
        
        for ( int i = 0; i < NUMBER_OF_GAMES; i++ )
        {
            maltipsButtons[ i ] = new TipsButton( ( i + 1 ) );
            maltipsButtons[ i ].addActionListener( singleSystemButtonViewControllerHandler );
    ...
    public void stateChanged( ChangeEvent event )
    {
        ...
        for ( int i = 0; i < NUMBER_OF_GAMES; i++ )
        {
            int value = maltipsDocument.getMaltipsSystem().getSingleSystem().getSingleSystemRow( i );

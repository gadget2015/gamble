...
public class AsmParameterNameLoader implements ParameterNameLoader {
    private static Logger LOGGER = Logger.getInstance(LogCategory.OPENEJB, AsmParameterNameLoader.class);
...
//                    }
    ...
    private static ClassReader createClassReader(Class declaringClass) throws IOException {
        ...
        try {
            ...
            if(classLoader == null) {
            	LOGGER.info("Some implementations may use null to represent the bootstrap class loader.");
            	classLoader = ClassLoader.getSystemClassLoader();            	
            }
            LOGGER.info("declaringClass=" + declaringClass);
            LOGGER.info("Name of class="+ declaringClass.getName());
            LOGGER.info("replaced=" + declaringClass.getName().replace('.', '/'));

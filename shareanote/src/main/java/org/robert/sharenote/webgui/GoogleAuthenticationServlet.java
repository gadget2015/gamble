...
import org.robert.sharenote.webgui.businessdelegate.NoteBusinessDelegateFactory;
...
import org.robert.sharenote.webgui.session.UserSessionFactoryServletImpl;
...
public class GoogleAuthenticationServlet extends HttpServlet {
	...
	NoteBusinessDelegate noteBusinessDelegate;
	UserSessionFactory userSessionFactory;

	public GoogleAuthenticationServlet() {
		NoteBusinessDelegateFactory factory = new NoteBusinessDelegateFactory();
		this.noteBusinessDelegate = factory.getRuntimeBD();
	}
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		boolean verificationResult = noteBusinessDelegate.verifyGoogleResponse(
		...
		if (verificationResult == true) {
			...
			try {
				...
				if (emails == null) {
					...
					response.sendRedirect(Navigation.AUTHENTICATION_ERROR_DIALOG);
				} else {
					...
					BrowserCheck browserChecker = new BrowserCheck();

					if (browserChecker.canViewShowInputRichText(request)) {
						response.sendRedirect(Navigation.START_GOOD);
					} else {
						response.sendRedirect(Navigation.START_SAFARI);
					}
				}
			} catch (IOException e) {

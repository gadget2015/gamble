...
public class UserSessionFactoryServletImpl implements UserSessionFactory {
	 ...
	 * assoc_handles in session because they are not unique per authentication
	 * request to google. If none exist create one. This is used because when
	 * authenticated with Goolge the session doesn't work properly, e.g the
	 * servlet can't get actors session.
	 */
	@Override
	public UserSession getUserSession() {
		...
		System.out.println("UserSessionFactory.getUserSession():# active HTTP sessions:"
		...
		for (HttpSession webSession : WebContainerSessionListener
			...
			System.out.println("Found correct type of session:"
					+ userSessionStorage);
